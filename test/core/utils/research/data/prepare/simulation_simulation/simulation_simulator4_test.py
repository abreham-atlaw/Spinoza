...
from core.utils.research.data.prepare.simulation_simulator.simulation_simulator4 import SimulationSimulator4
...
class SimulationSimulator4Test(unittest.TestCase):

	def setUp(self):
		df = pd.read_csv(os.path.join(BASE_DIR, "temp/Data/Al-All.10k.csv"))
		self.output_path = os.path.join(BASE_DIR, "temp/Data/simulation_simulator_data/10")
		...
		self.simulator = SimulationSimulator4(
			df=df,
			bounds=[
				load_json(os.path.join(Config.RES_DIR, "bounds/13.json")),
				load_json(os.path.join(Config.RES_DIR, "bounds/13.json")),
				load_json(os.path.join(Config.RES_DIR, "bounds/13.json")),

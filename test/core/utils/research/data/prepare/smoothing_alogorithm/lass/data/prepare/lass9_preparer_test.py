...
from core.utils.research.data.prepare.smoothing_algorithm.lass.data.prepare.lass9_preparer import Lass9Preparer
...
class Lass9PreparerTest(unittest.TestCase):
	...
	def test_functionality(self):
		...
		for f in np.random.randint(0, len(X_FILES), 10):
			...
			self.assertEqual(X.shape, (self.batch_size, len(self.x_columns), self.block_size))
			self.assertEqual(y.shape, (self.batch_size, len(self.y_columns), self.block_size))
			for idx, i in enumerate(np.random.randint(0, X.shape[0], 2)):
				plt.subplot(2, 2, idx + 1)
				plt.plot(X[i].transpose(1, 0), label=[f"X-{c}" for c in self.x_columns])
				plt.plot(y[i].transpose(1, 0), label=[f"Y-{c}" for c in self.y_columns])
			plt.show()

...
class TraderReflexMemoryEvaluatorTest(unittest.TestCase):
	...
	def __init_state(self, m: float, b: float, t: int):
		...
		market_state.update_state_of("AUD", "USD", np.arange(128).reshape((1, -1)))
		market_state.update_state_of("AUD", "USD", np.array([[128*m]]))
		...
		for i in range(t):
			agent_state.open_trade(TraderAction("AUD", "USD", TraderAction.Action.BUY, 20))
		return TradeState(
			market_state, agent_state
		)

	def setUp(self):
		self.evaluator = TraderReflexMemoryEvaluator()
		self.states = [
